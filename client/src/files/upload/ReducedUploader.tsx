...
export default class ReducedUploader extends React.PureComponent<Props> {
  render() {
    ...
    const loading = !!filesToUpload.find(fileToUpload => fileToUpload.loading)
    const indeterminate = progress === filesToUpload.length * 100 && loading
    const notUploaded = filesToUpload.filter(f => !f.done && !f.loading).length
    return (
        ...
        <ProgressBlock indeterminate={indeterminate} progress={progress} className={styles.reducedUploaderProgress}>
          <div className={styles.container}>
            <div className={styles.stats}>
              ...
              <div className={styles.counter}>{filesToUpload.filter(f => f.loading).length} Uploading</div>
              <div className={styles.counter}>{filesToUpload.filter(f => f.done).length} Completed</div>
  ...
  handleOnUpload = () => {
    ...
    filesToUpload.filter(f => !f.done && !f.loading).forEach(fileToUpload => {
...
export function totalProgress(filesToUpload: FileToUpload[]): number {
  const files = filesToUpload.filter(f => f.loading)
