...
import play.api.mvc.{BaseController, Request, Result, Results}
...
trait SerializationSupport
  ...
  trait Resulting[A] {
    ...
    def toResult(a: A)(implicit req: Request[_]): Result
  ...
  implicit val ResultingUnit: Resulting[Unit] =
    new Resulting[Unit] {
      ...
      def toResult(u: Unit)(implicit req: Request[_]): Result =
  ...
  implicit val ResultingResult: Resulting[Result] =
    new Resulting[Result] {
      ...
      def toResult(result: Result)(implicit req: Request[_]): Result =
        result
  ...
  implicit def ResultingEither[E, A](implicit rightResulting: Resulting[A], leftResulting: Resulting[E]): Resulting[Either[E, A]] =
    new Resulting[Either[E, A]] {
      ...
      def toResult(value: Either[E, A])(implicit req: Request[_]): Result =
        value match {
  ...
  implicit def ResultingOption[A](implicit resulting: Resulting[A]): Resulting[Option[A]] =
    new Resulting[Option[A]] {
      ...
      def toResult(value: Option[A])(implicit req: Request[_]): Result =
        value match {
          case Some(validated) =>
  ...
  protected def convertToFutureResult[A](value: Future[A], resulting: Resulting[A])(implicit req: Request[_]) : Future[Result] =
    value
      .map { value =>
  ...
  protected def convertToResult[A](value: A, resulting: Resulting[A])(implicit req: Request[_]) : Result =
  ...
  class ToFutureResultConverter[A](value: Future[A], resulting: Resulting[A]) {
    ...
    def toResult(implicit req: Request[_]) : Future[Result] =
  ...
  class ToResultConverter[A](value: A, resulting: Resulting[A]) {
    ...
    def toResult(implicit req: Request[_]) : Result =
