...
trait ErrorSupport extends SerializationSupport with I18nSupport with Logging {
  ...
  implicit val errorResultingWithLogging: Resulting[AppError] = new Resulting[AppError] {
    ...
    override def toResult(appError: AppError)(implicit req: RequestHeader): Result = {
  ...
  override def convertToFutureResult[A](value: Future[A], resulting: Resulting[A])(implicit req: RequestHeader): Future[Result] =
